...
# Abstract
A time series version of a data warehouse would enable a compact, agile and utra fast data wallet to built complete reports or complement exsiting ones. ?
?>Data warehouses provide a powerful solution but the main benefits are lost if additional data is required or data definitions do not match business
?>definitions.
