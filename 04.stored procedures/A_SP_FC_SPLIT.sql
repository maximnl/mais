...
CREATE  PROCEDURE [dbo].[A_SP_FC_SPLIT]
 @activity_id int = 0 
...
   		BEGIN 
        ...
        FROM dbo.A_TIME_DATE T INNER JOIN dbo.A_FACT_DAY F on F.date=T.date WHERE (' + @filter +')'
...
   		+ ', ' +  convert(nvarchar(max),@forecast_id) + ','+ convert(nvarchar(max),@import_id) 
        ...
        FROM dbo.A_TIME_DATE T INNER JOIN dbo.A_FACT_DAY F on F.date=T.date
